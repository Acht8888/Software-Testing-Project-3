                    ...
                    if "register successful" in test_type:
                        try:
                            if "success" in driver.current_url or "Created" in driver.page_source:
                                print(f"   -> [PASS] Register Successful (Redirected to Success Page).")
                                driver.get("https://ecommerce-playground.lambdatest.io/index.php?route=account/logout")
                            else:
                                print(f"   -> [FAIL] Expected Register Success but stayed on page.")
                        except: pass

                    elif expected_res == "Accepted":
                        if xp_error:
                            self.check_no_error(driver, xp_error)
                        else:
                            print(f"   -> [SKIP] Cannot verify 'Accepted' because error_xpath is missing in CSV.")

                    else:
                        if xp_error: 
                            self.check_error(driver, xp_error, expected_res)
                        else:
                            print(f"   -> [SKIP] Cannot verify Error because error_xpath is missing in CSV.")

        except FileNotFoundError:
            print("Lỗi: Không tìm thấy file CSV data_register_level2.csv.")
    ...
    def check_error(self, driver, xpath, expected):
        try:
            ...
            if expected in actual:
                ...
                print(f"   -> [FAIL] Mismatch! Expected: '{expected}' - Actual: '{actual}'")
        except NoSuchElementException:
            print(f"   -> [FAIL] Expected error '{expected}' but NO error message appeared.")

    def check_no_error(self, driver, xpath):
        try:
            driver.find_element(By.XPATH, xpath)
            print(f"   -> [FAIL] Unexpected error message appeared at this field!")
        except NoSuchElementException:
            print(f"   -> [PASS] Input Accepted (No error message displayed).")
